...
int N;  //그림 크기
int board[30][30];  //그림의 색깔 받는 배열
int group[900][900]={0,};   //그림의 인접한 같은 색끼리 그룹으로 표시할 배열
int groupSize[900]; //그룹이 이루어진 칸의 갯수 저장할 배열
int groupMapping[900];  //groupMappint[1] = 4 (1그룹의 실제 숫자는 4로 이루어져있다.)
int groupCnt;   //그룹이 만들어질 갯수 - 매번 초기화 해줘야 함

bool visited[30][30];   //그룹을 만들 때 이미 그룹으로 만들어진 색깔인지 표시할 배열
int dirX[] = {-1,0,1,0};
int dirY[] = {0,1,0,-1};
struct Point{
    int x, y;
...
void rotate_square(int x, int y){
    int len = (N-1)/2;
    int tmp[30][30] = {0,}; // board값을 이 배열에 먼저 
    //1 2 
    //3 4
